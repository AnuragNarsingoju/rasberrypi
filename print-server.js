...
// TSC Label Text Alignment Helper
class TSCTextAligner {
  constructor() {
    // Font width mapping for TSC fonts (approximate character widths in dots)
    this.fontWidths = {
      '0': 5,  '1': 5,  '2': 7,  '3': 10, '4': 14,
      '5': 19, '6': 8,  '7': 12, '8': 16
    };
  }

  // Calculate text width in dots
  calculateTextWidth(text, fontCode, xMul = 1) {
    const charWidth = this.fontWidths[fontCode] || 7;
    return text.length * charWidth * xMul;
  }

  // Calculate centered X position for horizontal text (0° rotation)
  centerTextHorizontal(text, fontCode, startX, endX, xMul = 1) {
    const textWidth = this.calculateTextWidth(text, fontCode, xMul);
    const availableWidth = endX - startX;
    return Math.round(startX + (availableWidth - textWidth) / 2);
  }

  // Calculate centered Y position for vertical text (270° rotation)
...
async function GiriviLabelPrint(labelData) {
  console.log('Girivi Label Print:', labelData);
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // PowerShell script for printing
      const psScript = `
...
`;
      ...
      const scriptPath = path.join(tempDir, `girivi-print-${Date.now()}.ps1`);
      ...
      exec(psCommand, async (error, stdout, stderr) => {
        ...
        if (stdout.includes('SUCCESS')) {
          resolve({
            success: true,
            message: 'Girivi label printed successfully',
            code: s_Code
          });
        } else {
          reject(new Error(stderr || 'Print operation failed'));
        }
      });

    } catch (err) {
      reject(new Error(`Girivi Label Print Error: ${err.message}`));
    }
  });
}
